...
def prep_faces(faceBytes, version):
    ...
    while len(faceBytes):
        ...
        if mds_pgon_type == 3:
            ...
            # Solid color / wire / none
            # TODO: Add color import/export
...
def build_bmesh(bm, sub, verts):
    ...
    for f in faces:
        ...
        try:
            ...
            bmface = bm.faces.new(extraVerts)
...
def make_mesh(subobject, verts, uvs):
    ...
    # EXPERIMENTAL
    # Face indices were not matching.
    for i in range(len(faces)):
        bmFace = bm.faces[i]
        binFace = faces[i]
        bmFace.index = binFace.index
    ...
    for s in subobjects:
        ...
        for v in s.vhots:
            if not options['support_3ds_export']:
                ...
                # Add custom properties for illum end trans
            else:
...
class Model(object):
    def __init__(self, root_sub, materials, bbox, clear, bright, use_node_image, Nodes_dict):
        ...
        self.max_jointid      = 1  # Should at least be 1.
        # TODO: Currently still wrong parent index, should be static to parent!.
        mat_flags = 0
        if clear:
            mat_flags += 1
        if bright:
        ...
        for bm in meshes:
            ...
            # Normals are sorted by their face index and unique if it matches.
            normal_data = [f[ext_f][:14] for f in bm.faces]
            # Doing this the ugly way without set - want to preserve order
            normal_data_set = []
            for o in normal_data:
                n = o[2:]
                if n not in normal_data_set:
                    normal_data_set.append(n)
            result = concat_bytes(normal_data_set)
            normals_bs += result
        ...
        for s in subs:
            ...
            for i in range(len(s.children)):
                ...
                if True: # Add behind
                    ...
                    node.node_behind = new_behind
                    node = new_behind
                    # Need to determine position/offset of the subobject nodes.
                else:
                    # in front
            ...
            for mds_pgon in s_pgons:
                ...
                # Key is face.index
                addr_dict[unpack('<H', mds_pgon[:2])[0]] = data
                ext_face_addrs += current_addr
                addr += len(mds_pgon)
        ...
        for idx, bm in enumerate(meshes):
            ...
            for node in Nodes_dict[bm]:
                ...
                if isinstance(node, SortingHelper.SplitNode) and node.norm == None:
                    ...
                    node.d = data[2]
                if not node.check():
                    print("Warning!:", node, "needs fixing!")     
        
        # Encode nodes
        # Let's do nodes, important for rendering as well
        ...
        for s in subs:
            main_node = Nodes_dict[s.mesh]
            s.num_nodes = 0
            
            print("Check subobj", s.index)            
            # Set node_off on parent pointing to this sub
            if s.index in split_nodes_onparent:
                parent_node, node = split_nodes_onparent[s.index]
                current_pos = sum(new_node_offs)
                node.position_offset = current_pos
                print("For Subobject storint:", current_pos)
                s.node_off = current_pos
                if parent_node.node_front == node:
                    parent_node.node_front_off = current_pos
                else:
                    parent_node.node_behind_off = current_pos
                del split_nodes_onparent[s.index]
            
            front_nodes_to_fill = []
            new_node_offs.append(3) # Size of subnode
            for node in main_node:
                s.num_nodes += 1
                current_pos = sum(new_node_offs)
                if type(node) != SortingHelper.SubobjectNode:
                ...
                if node in front_nodes_to_fill:
                    ...
                    print("Storing", current_pos, "for front node")
            ...
            if not g_NewNodes:
                ...
                sphere_bs = encode_sphere(get_local_bbox_data(meshes[si]))  # Cant we use obj earlier?
                ...
                if call:
                    ...
                    kids = s.children
                    node_offs = [sum(node_sizes[:a]) for a in range(len(node_sizes))]
                    ns = len(kids)
                    ...
                    encoded = encode_raw_node(nbf, sphere_bs, addr_chunks[si])
                    node_bs += encoded
        ...
        new_node_offs = [sum(new_node_sizes[:i+1]) for i in range(len(new_node_sizes)-1)]
        node_offs.insert(0, 0)
        new_node_offs.insert(0, 0)
        print("Node offs", node_offs)
        print("New offs", new_node_offs)
        print("new sizes:", new_node_sizes)
        ...
        for sub in subs:
            ...
            if jointid+1 >= self.max_jointid:
                self.max_jointid = jointid +1
            print(jointid)
...
def find_d(n, vs):
    # Plane equation that is used:
    # n @ v = d
    # n @ v - d = 0
    # For split planes they seam to use the top formula? / they reverse the normal
    # Packing -d into nodes.
    nx, ny, nz = n
    count = len(vs)
...
"""
def encode_call_node(nf, offset, sphere_bs, addr_chunk):
    return pack('<B16s3H', 2, sphere_bs, nf, offset, 0) + addr_chunk
...
def encode_header(model, offsets):
    ...
    return concat_bytes([
        b'LGMD',            # Type of model "LGMD" static, "LGMM" mesh
        b'\x04\x00\x00\x00',# // version of bsp that produced it, copy of original bsp
        model.names[0],
        pack('<f', radius),
        pack('<f', model.max_poly_radius),
        encode_floats(model.bbox[max]),
        encode_floats(model.bbox[min]),
        bytes(12),          # relative centre, // centroid pos relative to parent
        encode_ushorts([
            model.num_faces,
            model.num_verts,
            model.max_jointid]),  # >6 has not effect < see TWEQCTRL.CPP
        encode_ubytes([
...
def encode_material(binName, index, color=0):
    #print(binName, index)
    #if color:
    ...
    return concat_bytes([
        binName,
        pack('B', color),   # material type 0= texture,
        pack('B', index),   # // slot to put it in
        bytes(4),           # ??? // "texture handle or argb" 
        bytes(4)])          # ??? // "uv mip map OR ipal" # see above
...
def prep_subs(all_objs, materials, use_origin, sorting):
    ...
    for g3po, g3pm, g3pvh in zip(gen3_plus, gen3_plus_meshes, vhots[2]):
        ...
        obj_lookup[g3po] = sub
    return root_sub, bbox, Nodes_dict
...
"""
#define MD_PGON_PRIM_MASK  0x07 = 7
#define MD_PGON_LIGHT_ON   0x18 = 24
#define MD_PGON_COLOR_MASK 0x60 = 96
...
#define MD_PGON_COLOR_PAL  0x20 = 32 // palette color
#define MD_PGON_COLOR_VCOL 0x40 = 64 // vcolor lookup
...
typedef struct mds_pgon_aux {
   ubyte   mat; // which material # Is used for mat_aux data
} mds_pgon_aux;
"""

def encode_pgon(f, f_idx, tx_idx, n_idx, vs, lts, uvs,
                bright=False, drawstyle='TMAP', color=None):
    data = tx_idx
    type = {'NONE': 0,
            'SOLID': 1,
            'WIRE' : 2,
    ...
    if not bright:
        type    +=  0x18
    if color:
        print(f.index, "gets colored")
        # palett index or so, can't get it to be valid.
        # only ushort
        data    = color
    num_vs = len(f.verts)
    #n_idx already passed
    d = find_d(f.normal, [v.co[:] for v in f.verts])
    verts = vs + lts + uvs
    return pack('<HHBBHf', f_idx, data, type, num_vs, n_idx, d) + \
            verts + pack('B', tx_idx) 

def extend_faces(offs, bm, materials):
    ...
    for f in bm.faces:
        ...
        n_set.add(n_bs)
        if n_bs not in new_n_dict:
            new_n_dict[n_bs] = f.index - idx_adjuster
        else:
            idx_adjuster += 1 # So if 2 faces have the same cant continue with f.index
        f[ext_f] = n_bs
    num_ns = len(n_set)
    n_dict = dict(zip(n_set, range(num_ns)))
    for f in bm.faces:
        f_idx = f_off + f.index # Need to increase for other meshes
        n_bs = f[ext_f]
        n_idx = n_off + new_n_dict[n_bs]
        ...
        tx = f.material_index
        # Posible for the future to extract stuff like color.
        options = {}
        if len(materials):   # only fails on empty models
            mat = materials[tx]
            if "bright" in mat:
                options.update(bright=True)
        mds_pgon = encode_pgon(f, f_idx, tx, n_idx, vs, lts, uvs, **options)
        f[ext_f] = pack('>H', n_idx) + n_bs + mds_pgon
...
def reorder_faces(bm, order):
    ...
    for fi in order:
        ...
        bm.faces.index_update()
    bm.normal_update()
    # EXPERIMENTAL #TODO: is this dangerous. Would like to access them via index for nodes.
    bm.faces.ensure_lookup_table() 
    bm0.free()

###
### Sorting: Normal based
###
